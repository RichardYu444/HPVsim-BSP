...
class Sim(hpb.BaseSim):
    ...
    def run(self, do_plot=False, until=None, restore_pars=True, reset_seed=True, verbose=None, **kwargs):
        ...
        # Make sure people exist before checking their time index
        if (self.people is not None) and (self.people.t not in [self.t, self.t-1]):  # Depending on how the sim stopped, either of these states are possible
